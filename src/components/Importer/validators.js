...
export const applyValidation = (formattedData, fields) => {
  ...
  fields.forEach((field) => {
    validatorsByFieldKey[field.accessor] = [];
    if (!field.validators) {
      return;
    }

    field.validators.forEach((validatorDefinition) => {
      validatorsByFieldKey[field.accessor].push(
  ...
  fields.forEach((field) => {
    formattedData.forEach((row, rowIndex) => {
      if (!(field.accessor in row) && !fieldIsRequired(field)) {
        return;
      }
      const value = row[field.accessor];
      const validators = validatorsByFieldKey[field.accessor];

      validators.forEach((v) => {
        const result = v.isValid(value);
        if (!result.valid) {
          validationResult.addError(field.accessor, rowIndex, result);
        }
      });
    });
  });
...
export const computeStatistics = (
  ...
  headers.forEach((headerMapping) => {
    ...
    const fieldKey = headerMapping.selectedField.accessor;
